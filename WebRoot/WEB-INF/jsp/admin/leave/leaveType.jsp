		...
		$(function(){
			...
			$("#addLeaveType").click(function(){
				$("#dialogDiv").dialog({
					...
					width: "350",
					height: "220",
					//href: "${path}/admin/leave/showAddLeaveType.jspx",
					closed: false,
					collapsible: true,
					minimizable: true,
					maximizable: true,
					...
					onClose: function(){
						/** 对话框关闭后，重新查询假期类型列表 */
						window.location.href="${path }/admin/leave/selectLeaveType.jspx?pageModel.pageIndex=${pageModel.pageIndex}&leaveType.code=${leaveType.code}";
						/** 刷新左边假期类型树 */
						parent.leaveTypeLeftFrame.location.reload();
					}
				});
				$("#iframe").attr("src","${path}/admin/leave/showAddLeaveType.jspx?").show();
			});
			...
			$("#updateLeaveType").click(function(){
				...
				if(boxs.length == 0){
					...
					$("#dialogDiv").dialog({
						...
						width: "350",
						height: "220",
						//href: "${path}/admin/leave/showUpdateLeaveType.jspx",
						closed: false,
						collapsible: true,
						minimizable: true,
						maximizable: true,
					...
					$("#iframe").attr("src","${path}/admin/leave/showUpdateLeaveType.jspx?leaveType.code=" + boxs.val()).show();
				}else{
					alert("只能勾选一条记录进行修改");
				}
			});
			...
			$("#deleteLeaveType").click(function(){
				...
				if(boxs.length == 0){
					...
					if(confirm("确定删除吗？")){
						...
						window.location.href="${path }/admin/leave/deleteLeaveType.jspx?pageModel.pageIndex=${pageModel.pageIndex}&leaveType.code=${leaveType.code}&codes=" + codes.join(",");
						/** 刷新左边假期类型树 */
						parent.leaveTypeLeftFrame.location.reload();
					}
...
<body>
	<!-- 工具按钮区 -->
		<table>
			<tr>
				<td><input type="button" value="添加" id="addLeaveType"/></td>
				<td><input type="button" value="修改" id="updateLeaveType"/></td>
				<td>
					<input type="button" value="删除" id="deleteLeaveType"/>
					<!-- 删除成功后的提示信息 -->
	...
	<table width="100%" class="listTable" cellpadding="8" cellspacing="1">
		...
		<tbody style="background-color: #FFFFFF;">
			<s:iterator value="leaveTypes" status="st">
	...
	<jsoft:pager 
		...
		submitUrl="${path }/admin/leave/selectLeaveType.jspx?pageModel.pageIndex={0}"/>
