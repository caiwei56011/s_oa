/****************** 创建数据库*****************/
-- create database oa_db default character set utf8; 
...
# 模块code去重复
select module_code from oa_id_popedom;
-- 方式一:
select module_code from oa_id_popedom group by module_code;
-- 方式二：
select distinct module_code from oa_id_popedom;

# 根据角色id查询绑定到当前角色的用户
select * from oa_id_user a 
	join oa_id_user_role b
...
# 根据角色id查询未绑定到当前角色的用户
/*下面语句写法有问题，因为一个用户可以对应多个角色，
 *可能中间表角色id=2的用户中也包含这个用户 
 */
select * from oa_id_user a 
	join oa_id_user_role b
	on a.user_id=b.USER_ID
 ...
 * 应该用子查询的方式：
 */
 select * from oa_id_user a where a.user_id not in 
	(select b.user_id from oa_id_user b 
		join oa_id_user_role c
		on b.user_id=c.USER_ID
		join oa_id_role d
		on c.ROLE_ID = d.id
		where d.id=1)
	order by a.create_date asc;
# 根据用户id查询他所有的角色，再根据角色查询他所有的有权限的操作模块
/** 写法一：子查询 */
select 	a.module_code from oa_id_popedom a  
	where a.role_id in
	(
	  select b.role_id from oa_id_user_role b
		join oa_id_user c
		on b.USER_ID=c.user_id
